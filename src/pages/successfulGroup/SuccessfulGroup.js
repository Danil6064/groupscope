<<<<<<< HEAD
import React from 'react';
import './successfulGroup.css'

function SuccessfulGroup() {
    return (
<main className="main_successfulGroup">

<div className="container_successfulGroup">

<div className="dropdown-menu">
<div className="dropdown-menu-btn">
<svg width="15" height="15" viewBox="0 0 19 19">
<polygon points="0,0 19,0 9.5,19" />
</svg>
<h2>ПКр</h2>
<svg width="15" height="15" viewBox="0 0 19 19">
<polygon points="0,0 19,0 9.5,19" />
</svg>
</div>
<div className="dropdown-menu-elements">
<ul>
<li>акс</li>
<li>електрорадіовимірювання</li>
<li>ООП</li>
<li>комплекси ТЗІ</li>
<li>інформаційні технології</li>
<li>ікс</li>
</ul>
</div>
</div>


    <div className="table__header-row">

        <div className="table-header-row__designation">
            <div className="table__center-container">
                <div className="table__red-circle"></div>
                <span>Не зроблено</span>
            </div>
            <div className="table__center-container">
                <div className="table__yellow-circle"></div>
                <span>Зроблено</span>
            </div>
        </div>

        <ul className="table-header-row__headings">
            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№3</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№4</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№5</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№6</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№7</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№8</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№3</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№4</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№5</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№6</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№7</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>тест</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>тест</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>кр</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>кр</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__conclusion table__even-column">
                <span>Висновок</span>
            </li>
        </ul>

    </div>

    <div className="table">

        <div className="table__row table__first-row">
            <div className="table-row__student-name">
                1. Архіпов Назар Романович
            </div>
            <ul className="table-row__marks ">

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__conclusion table__even-column">
                    <div className="table__red-conclusion"></div>
                </li>
            </ul>
        </div>


        <div className="table__row">
            <div className="table-row__student-name">
                2. Ашагіре Даніель Шева
            </div>
            <ul className="table-row__marks">

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__conclusion table__even-column">
                    <div className="table__yellow-conclusion"></div>
                </li>
            </ul>
        </div>


        <div className="table__row">
            <div className="table-row__student-name">
                3. Блюма Костянтин Андрійович
            </div>
            <ul className="table-row__marks">

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__conclusion table__even-column">
                    <div className="table__gradient-conclusion"></div>
                </li>
            </ul>
        </div>


        <div className="table__row">
            <div className="table-row__student-name">
                8. Лукіянчук Олександр Костянтинович
            </div>
            <ul className="table-row__marks">

                <li className="table-row__mark table__odd-column">
                    60
                </li>

                <li className="table-row__mark table__even-column">
                    60
                </li>

                <li className="table-row__mark table__odd-column">
                    60
                </li>

                <li className="table-row__mark table__even-column">
                    60
                </li>

                <li className="table-row__mark table__odd-column">
                    80
                </li>

                <li className="table-row__mark table__even-column">
                    80
                </li>

                <li className="table-row__mark table__odd-column">
                    80
                </li>

                <li className="table-row__mark table__even-column">
                    100
                </li>

                <li className="table-row__mark table__odd-column">
                    100
                </li>

                <li className="table-row__mark table__even-column">
                    100
                </li>

                <li className="table-row__mark table__odd-column">
                    90
                </li>

                <li className="table-row__mark table__even-column">
                    95
                </li>

                <li className="table-row__mark table__odd-column">
                    85
                </li>

                <li className="table-row__mark table__even-column">
                    75
                </li>

                <li className="table-row__mark table__odd-column">
                    65
                </li>

                <li className="table-row__mark table__even-column">
                    88
                </li>

                <li className="table-row__mark table__odd-column">
                    65
                </li>

                <li className="table-row__mark table__even-column">
                    88
                </li>

                <li className="table-row__mark table__odd-column">
                    77
                </li>

                <li className="table-row__conclusion table__even-column">
                    99
                </li>
            </ul>
        </div>

    </div>


    <div className="table__header-row">
        <div className="table-header-row__designation bottom-designation">
            <div className="table__center-container">
                <div className="table__red-circle"></div>
                <span>Не зроблено</span>
            </div>
            <div className="table__center-container">
                <div className="table__yellow-circle"></div>
                <span>Зроблено</span>
            </div>
        </div>

        <ul className="table-header-row__headings bottom-headings">
            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№3</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№4</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№5</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№6</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№7</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№8</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№3</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№4</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№5</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№6</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№7</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>тест</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>тест</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>кр</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>кр</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__conclusion table__even-column">
                <span>Висновок</span>
            </li>
        </ul>

    </div>

</div>

</main>

    )
}

=======
import React from 'react';
import './successfulGroup.css'

function SuccessfulGroup() {
    return (
<main className="main_successfulGroup">

<div className="container_successfulGroup">

<div className="dropdown-menu">
<div className="dropdown-menu-btn">
<svg width="15" height="15" viewBox="0 0 19 19">
<polygon points="0,0 19,0 9.5,19" />
</svg>
<h2>ПКр</h2>
<svg width="15" height="15" viewBox="0 0 19 19">
<polygon points="0,0 19,0 9.5,19" />
</svg>
</div>
<div className="dropdown-menu-elements">
<ul>
<li>акс</li>
<li>електрорадіовимірювання</li>
<li>ООП</li>
<li>комплекси ТЗІ</li>
<li>інформаційні технології</li>
<li>ікс</li>
</ul>
</div>
</div>


    <div className="table__header-row">

        <div className="table-header-row__designation">
            <div className="table__center-container">
                <div className="table__red-circle"></div>
                <span>Не зроблено</span>
            </div>
            <div className="table__center-container">
                <div className="table__yellow-circle"></div>
                <span>Зроблено</span>
            </div>
        </div>

        <ul className="table-header-row__headings">
            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№3</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№4</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№5</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№6</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№7</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№8</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№3</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№4</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№5</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№6</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№7</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>тест</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>тест</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>кр</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>кр</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__conclusion table__even-column">
                <span>Висновок</span>
            </li>
        </ul>

    </div>

    <div className="table">

        <div className="table__row table__first-row">
            <div className="table-row__student-name">
                1. Архіпов Назар Романович
            </div>
            <ul className="table-row__marks ">

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__conclusion table__even-column">
                    <div className="table__red-conclusion"></div>
                </li>
            </ul>
        </div>


        <div className="table__row">
            <div className="table-row__student-name">
                2. Ашагіре Даніель Шева
            </div>
            <ul className="table-row__marks">

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__conclusion table__even-column">
                    <div className="table__yellow-conclusion"></div>
                </li>
            </ul>
        </div>


        <div className="table__row">
            <div className="table-row__student-name">
                3. Блюма Костянтин Андрійович
            </div>
            <ul className="table-row__marks">

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__yellow-circle"></div>
                </li>

                <li className="table-row__mark table__even-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__mark table__odd-column">
                    <div className="table__red-circle"></div>
                </li>

                <li className="table-row__conclusion table__even-column">
                    <div className="table__gradient-conclusion"></div>
                </li>
            </ul>
        </div>


        <div className="table__row">
            <div className="table-row__student-name">
                8. Лукіянчук Олександр Костянтинович
            </div>
            <ul className="table-row__marks">

                <li className="table-row__mark table__odd-column">
                    60
                </li>

                <li className="table-row__mark table__even-column">
                    60
                </li>

                <li className="table-row__mark table__odd-column">
                    60
                </li>

                <li className="table-row__mark table__even-column">
                    60
                </li>

                <li className="table-row__mark table__odd-column">
                    80
                </li>

                <li className="table-row__mark table__even-column">
                    80
                </li>

                <li className="table-row__mark table__odd-column">
                    80
                </li>

                <li className="table-row__mark table__even-column">
                    100
                </li>

                <li className="table-row__mark table__odd-column">
                    100
                </li>

                <li className="table-row__mark table__even-column">
                    100
                </li>

                <li className="table-row__mark table__odd-column">
                    90
                </li>

                <li className="table-row__mark table__even-column">
                    95
                </li>

                <li className="table-row__mark table__odd-column">
                    85
                </li>

                <li className="table-row__mark table__even-column">
                    75
                </li>

                <li className="table-row__mark table__odd-column">
                    65
                </li>

                <li className="table-row__mark table__even-column">
                    88
                </li>

                <li className="table-row__mark table__odd-column">
                    65
                </li>

                <li className="table-row__mark table__even-column">
                    88
                </li>

                <li className="table-row__mark table__odd-column">
                    77
                </li>

                <li className="table-row__conclusion table__even-column">
                    99
                </li>
            </ul>
        </div>

    </div>


    <div className="table__header-row">
        <div className="table-header-row__designation bottom-designation">
            <div className="table__center-container">
                <div className="table__red-circle"></div>
                <span>Не зроблено</span>
            </div>
            <div className="table__center-container">
                <div className="table__yellow-circle"></div>
                <span>Зроблено</span>
            </div>
        </div>

        <ul className="table-header-row__headings bottom-headings">
            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№3</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№4</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№5</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№6</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>пз</span>
                <span>№7</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>пз</span>
                <span>№8</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№3</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№4</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№5</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>лб</span>
                <span>№6</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>лб</span>
                <span>№7</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>тест</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>тест</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__task-type table__even-column">
                <span>кр</span>
                <span>№1</span>
            </li>

            <li className="table-header-row__task-type table__odd-column">
                <span>кр</span>
                <span>№2</span>
            </li>

            <li className="table-header-row__conclusion table__even-column">
                <span>Висновок</span>
            </li>
        </ul>

    </div>

</div>

</main>

    )
}

>>>>>>> 9c66d1cf54a1d5acc5699260c428742d2f684960
export default SuccessfulGroup